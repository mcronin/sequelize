...
module.exports = (function() {
  ...
  */
  ...
  AbstractQuery.prototype.isBaseTableQuery = function() {
    return this.options.type === QueryTypes.BASETABLE;
  };
  ...
  AbstractQuery.prototype.handleSelectQuery = function(results) {
    ...
    } else if (this.options.hasMultiValuedColumns) {
      var self = this
      return new Promise(function(resolve) {
        return resolve(Promise.map(results, function(row) {
          return doSubQueries.call(self, row);
        }));
      });
      // Regular queries
    } else {
  ...
  */
  ...
  var doSubQueries = function(row) {
    var self = this;
    return new Promise(function(resolve, reject) {
      Promise.map(self.options.include, function(include) {
        var model = include.model
          , where = {};
        self.callee.primaryKeyAttributes.forEach(function(primaryKey) {
          where[model.rawAttributes[primaryKey].field] = row[primaryKey];
        });
        return model.sequelize.queryInterface.select(
          ...
          var originalAttributes = include.originalAttributes;
          originalAttributes.forEach(function(attr) {
            row[attr] = [];
          });
          results.forEach(function(subRow) {
            Object.keys(subRow).forEach(function(attr) {
              if (originalAttributes.indexOf(attr) !== -1) {
                row[attr].push(subRow[attr]);
              }
            });
          });
        });
      }).then(function(results) {
        if (self.options.originalAttributes) {
          Object.keys(row).forEach(function(attr) {
            if (self.options.originalAttributes.indexOf(attr) === -1) {
              delete row[attr];
            }
          });
        }
