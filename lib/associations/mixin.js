...
var Utils = require('./../utils')
  ...
  , MultiValuedColumn = require('./multi-valued-column');
...
/**
 * Create an association that is either 1:m and automatatic.
 *
 * ```js
 * // Create a 1:m association between user and project
 * User.multiValuedColumn(Project)
 * ```
 * By default, the name of the join table will be source+target, so in this case projectsusers. This can be overridden by providing either a string or a Model as `through` in the options.
 ...
 * })
 * User.multiValuedColumn(Project)
 * Project.multiValuedColumn(User)
 * ```
 * ```js
 * jan.addProject(homework, { started: false }) // The homework project is not started yet
 * jan.setProjects([makedinner, doshopping], { started: true}) // Both shopping and dinner has been started
 ...
 * @param {boolean}             [options.constraints=true] Should on update and on delete constraints be enabled on the foreign key.
 */
Mixin.multiValuedColumn = function(targetModel, options) {
  if (!(targetModel instanceof this.sequelize.Model)) {
    throw new Error(this.name + ".multiValuedColumn called with something that's not an instance of Sequelize.Model");
  }
  ...
  // the id is in the foreign table or in a connecting table
  var association = new MultiValuedColumn(sourceModel, targetModel, options);
  sourceModel.associations[association.associationAccessor] = association.injectAttributes();
