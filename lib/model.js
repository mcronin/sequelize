...
module.exports = (function() {
  ...
  var Model = function(name, attributes, options) {
    ...
    this.associations = {};
    this.hasMultiValuedColumns = false;
  ...
  Model.prototype.refreshAttributes = function() {
    ...
    Utils._.each(this.rawAttributes, function(definition, name) {
      ...
      } else if (definition.type instanceof DataTypes.DATE || definition.type instanceof DataTypes.TIME) {
  ...
  Model.prototype.findAll = function(options, queryOptions) {
    ...
    return Promise.bind(this).then(function() {
      ...
      if (typeof options === 'object') {
        ...
        var multiValuedColumns = [];
        
       if (!this.include && this.hasMultiValuedColumns) {
          
         options.include = [];
         options.hasMultiValuedColumns = true;
          ...
          options.attributes.forEach(function(attr) {
            if (self.rawAttributes[attr].multiValued) {
              multiValuedColumns.push(attr);
              var exists = false,
                model = self.modelManager.getDAO(self.rawAttributes[attr].multiValued);
              options.include.forEach(function(include) {
                if (!exists && include.model == model) {
                  exists = true;
                  include.attributes.push(attr);
                }
              });
              if (!exists) {
                options.include.push({ model: model, attributes: [ attr ] });
              }
            } else {
          ...
          if (options.include.length === 0) {
            ...
            options.hasMultiValuedColumns = false;
          }
        } else {
         options.hasMultiValuedColumns = false;        
        }
        ...
        if (options.include) {
          hasJoin = ! options.hasMultiValuedColumns;
      ...
      if (options.originalAttributes && options.hasMultiValuedColumns) {
        options.originalAttributes = options.originalAttributes.concat(multiValuedColumns);
      }
